...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.integrationSystem.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.integration-systems.update", [$integrationSystem->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="system_name">{{ trans('cruds.integrationSystem.fields.system_name') }}</label>
                <input class="form-control {{ $errors->has('system_name') ? 'is-invalid' : '' }}" type="text" name="system_name" id="system_name" value="{{ old('system_name', $integrationSystem->system_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('username') ? 'is-invalid' : '' }}" type="text" name="username" id="username" value="{{ old('username', $integrationSystem->username) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('password') ? 'is-invalid' : '' }}" type="text" name="password" id="password" value="{{ old('password', $integrationSystem->password) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('system_status') ? 'is-invalid' : '' }}" name="system_status" id="system_status" required>
                    ...
                    @foreach(App\Models\IntegrationSystem::SYSTEM_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('system_status', $integrationSystem->system_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
