    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.vouchers.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('voucher_status') ? 'is-invalid' : '' }}" name="voucher_status" id="voucher_status" required>
                    ...
                    @foreach(App\Models\Voucher::VOUCHER_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('voucher_status', 'Not Used') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('expired_at') ? 'is-invalid' : '' }}" type="text" name="expired_at" id="expired_at" value="{{ old('expired_at') }}">
