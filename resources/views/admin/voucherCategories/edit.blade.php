    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.voucher-categories.update", [$voucherCategory->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('voucher_name') ? 'is-invalid' : '' }}" type="text" name="voucher_name" id="voucher_name" value="{{ old('voucher_name', $voucherCategory->voucher_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $voucherCategory->amount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('currency') ? 'is-invalid' : '' }}" name="currency" id="currency">
                    ...
                    @foreach(App\Models\VoucherCategory::CURRENCY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('currency', $voucherCategory->currency) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
