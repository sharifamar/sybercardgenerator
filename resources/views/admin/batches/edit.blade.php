    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.batches.update", [$batch->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('batch_serial_number') ? 'is-invalid' : '' }}" type="text" name="batch_serial_number" id="batch_serial_number" value="{{ old('batch_serial_number', $batch->batch_serial_number) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('expiry_date') ? 'is-invalid' : '' }}" type="text" name="expiry_date" id="expiry_date" value="{{ old('expiry_date', $batch->expiry_date) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('voucher') ? 'is-invalid' : '' }}" name="voucher_id" id="voucher_id" required>
                    @foreach($vouchers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('voucher_id') ? old('voucher_id') : $batch->voucher->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('number_of_vouchers') ? 'is-invalid' : '' }}" type="number" name="number_of_vouchers" id="number_of_vouchers" value="{{ old('number_of_vouchers', $batch->number_of_vouchers) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('voucher_status') ? 'is-invalid' : '' }}" name="voucher_status" id="voucher_status">
                    ...
                    @foreach(App\Models\Batch::VOUCHER_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('voucher_status', $batch->voucher_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('generated') ? 'is-invalid' : '' }}" name="generated" id="generated" required>
                    ...
                    @foreach(App\Models\Batch::GENERATED_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('generated', $batch->generated) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
